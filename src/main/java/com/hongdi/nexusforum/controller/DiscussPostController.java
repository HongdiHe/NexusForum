...
import com.hongdi.nexusforum.entity.*;
import com.hongdi.nexusforum.event.EventProducer;
import com.hongdi.nexusforum.service.*;
...
public class DiscussPostController implements CommunityConstant {
    ...
    public String getDiscussPost(@PathVariable("discussPostId") int discussPostId, Model model, Page page) {
        ...
        // Batch load all users referenced in comments and replies to avoid N+1 queries
        Set<Integer> allUserIds = new HashSet<>();
        Map<Comment, List<Comment>> commentReplyMap = new LinkedHashMap<>();
        if (commentList != null) {
            for (Comment comment : commentList) {
                allUserIds.add(comment.getUserId());
                List<Comment> replyList = commentService.findCommentsByEntity(
                        ENTITY_TYPE_COMMENT, comment.getId(), 0, Integer.MAX_VALUE);
                commentReplyMap.put(comment, replyList);
                if (replyList != null) {
        ...
        Map<Integer, User> userMap = userService.findUsersByIds(allUserIds);
        ...
        if (commentList != null) {
            for (Comment comment : commentList) {
                ...
                // 回复列表 (already fetched during batch collection)
                List<Comment> replyList = commentReplyMap.get(comment);
                // 回复VO列表
